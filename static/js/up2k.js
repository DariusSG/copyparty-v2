...
function up2k_init(subtle) {
    ...
    setTimeout(function () {
        if (window.WebAssembly && !hws.length)
            fetch(serverRootURL + '/static/js/w.hash.js');
    }, 1000);
    ...
    function init_deps() {
        if (!loading_deps && !got_deps()) {
            ...
            import_js(serverRootURL + '/.cpr/deps/' + fn, unmodal);
    ...
    function up_them(good_files) {
        ...
        if (window.WebAssembly && !hws.length) {
            for (var a = 0; a < Math.min(navigator.hardwareConcurrency || 4, 16); a++)
                hws.push(new Worker(serverRootURL + '/static/js/w.hash.js'));
